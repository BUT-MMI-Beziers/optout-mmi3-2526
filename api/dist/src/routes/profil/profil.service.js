...
export async function updateProfil(userId, body) {
    const updates = {
        updatedAt: new Date(),
    };
    if (body.firstName !== undefined)
        updates.firstName = encrypt(body.firstName);
    if (body.lastName !== undefined)
        updates.lastName = encrypt(body.lastName);
...
export async function addContact(userId, body) {
...
export async function deleteContact(userId, contactId) {
    ...
    const min = CONTACT_LIMITS[contact.type].min;
    if (min > 0) {
